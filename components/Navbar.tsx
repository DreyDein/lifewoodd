...
const Navbar: React.FC<NavbarProps> = ({ onApplyClick, currentLang, onLangChange, currentTheme, onThemeToggle, t }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 py-3 px-6 md:px-12 border-b ${
      scrolled 
      ...
      <div className="max-w-[1400px] mx-auto flex justify-between items-center">
        ...
        <button 
          ...
          className={`lg:hidden p-2 focus:outline-none z-50 transition-colors duration-300 ${textColorClass}`}
        >
          {isMenuOpen ? (
      ...
      {isMenuOpen && (
        <div 
          className="fixed inset-0 bg-lifewood-dark/60 backdrop-blur-sm z-40 lg:hidden"
          onClick={() => setIsMenuOpen(false)}
        />
      )}

      {/* Mobile Menu Panel */}
      <div className={`fixed top-0 right-0 h-full w-full max-w-xs bg-lifewood-white dark:bg-lifewood-dark z-50 transition-transform duration-300 lg:hidden ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} shadow-2xl overflow-y-auto`}>
        {/* Close Button */}
        <button
          onClick={() => setIsMenuOpen(false)}
          className="absolute top-6 right-6 p-2 text-lifewood-dark dark:text-lifewood-seaSalt hover:opacity-70 transition-opacity z-10"
        ...
        <div className="flex flex-col h-full pt-24 px-8 space-y-6">
          ...
          {navLinks.map((item) => (
            <a key={item.name} href={item.href} onClick={(e) => { scrollToSection(e, item.href); setIsMenuOpen(false); }} className="text-2xl font-bold text-lifewood-green dark:text-lifewood-seaSalt border-b border-lifewood-paper dark:border-lifewood-green/20 pb-2">
              {item.name}
            </a>
          ))}
          <div className="pt-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {languages.map(l => (
                <button key={l.code} onClick={() => onLangChange(l.code as Language)} className={`p-2 text-sm rounded-lg border ${currentLang === l.code ? 'border-lifewood-green bg-lifewood-green/5 text-lifewood-green' : 'border-lifewood-paper dark:border-lifewood-green/20 text-lifewood-dark/60 dark:text-lifewood-seaSalt/60'}`}>
                  {l.label}
                </button>
              ))}
            </div>
          </div>
          <button onClick={() => { onApplyClick(); setIsMenuOpen(false); }} className="bg-lifewood-green text-lifewood-white text-center py-4 rounded-xl font-bold text-xl mt-4">
            {t.apply}
          </button>
        </div>
      </div>
